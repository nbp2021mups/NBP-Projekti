...
export class AuthService {
    ...
    autoLogin() {
        ...
        const loggedUser: {id: number, _token: string, _tokenExpDate: Date} = JSON.parse(localStorage.getItem('logged-user'));
        if (!loggedUser) {
            return;
        }
        
        const user = new User(loggedUser.id, loggedUser._token, new Date(loggedUser._tokenExpDate));
        if(!user.token){
            return;
        }

        this.user.next(user);
        const expTimer = new Date(loggedUser._tokenExpDate).getTime() - new Date().getTime();
